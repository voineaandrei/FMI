...
train_labels = np.append(train_labels, np.loadtxt("validation_labels.txt", dtype = int, usecols = 1))
...
# ACEST NAIVE BAYES INDIVIDUAL ESTE ȘI O SOLUȚIE DIN CELE DOUĂ SUBMISII (cu fix acești parametri)
...
# analog logistic regression doar că folosește celelalte date
...
sol = np.array([test_id[:], predictions_naive_bayes[:]])
sol = sol.T
np.savetxt("solutie.csv", sol, delimiter=",", fmt='%s', header='id,label', comments='')
